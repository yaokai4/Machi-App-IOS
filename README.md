...
> 本仓库即 Xcode 工程中名为 `Machi` 的 **file-system-synchronized 源码组**（App 源码 + 资源）。
...
**环境要求**：macOS + **Xcode 26 或更高**（需 iOS 26 SDK）。

> ⚠️ **仓库范围说明**：本仓库托管的是 Xcode 工程里名为 `Machi` 的 **App 源码组**（采用 Xcode 16+ 的 file-system-synchronized groups）。
> 为保持仓库聚焦，`Machi.xcodeproj` 工程文件以及 `kaiziTests/`、`kaiziUITests/` 测试 target 目录位于工程根目录、**不在本仓库内**。
> 因此**直接 clone 本仓库无法独立编译**，需要配合工程文件使用。

获得可编译工程的两种方式：

- **A（已有工程）**：把本仓库作为 `Machi/` 文件夹，放在 `Machi.xcodeproj` 同级目录下，使其与同步组 `path = Machi` 对应，然后打开 `Machi.xcodeproj`。
- **B（新建工程）**：用 Xcode 26 新建一个 iOS App 工程（最低 iOS 26、Swift、SwiftUI 生命周期），删除模板源文件后，用「file-system-synchronized group」引用本仓库目录即可。

运行步骤：

1. 用 Xcode 打开工程，选中 `Machi` target。
2. 在 **Signing & Capabilities** 里把 Team 改成你自己的开发者账号（仓库中的 `DEVELOPMENT_TEAM = P22K8NF89K` 仅对原作者有效）。
3. 选择 iOS 26 模拟器或真机，`Cmd + R` 运行。
4. 启动统一后端（见 `machi-web` 仓库的 `python3 server.py`），或在 App 内 / `Info.plist` 把 `KAIX_API_BASE` 指向你的后端地址。
