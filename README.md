...
| 最低系统 | iOS 17.0（当前用 iOS 26.5 模拟器验证） |
| 构建工具 | Xcode 26+（iOS 26 SDK 已验证） |
...
**环境要求**：macOS + **Xcode 26 或更高**（iOS 26 SDK 已验证；App 的最低系统版本为 iOS 17.0）。仓库即标准 iOS 工程，clone 后可直接编译。
...
2. 在 Organizer 中校验签名、`PrivacyInfo.xcprivacy` 和 capabilities，上传到 App Store Connect。
3. 确认 App ID / provisioning profile 已开启 Push Notifications 与 Sign in with Apple；Release 使用 `APS_ENVIRONMENT = production`。
4. 发布前确认生产环境的 `KAIX_API_BASE` 指向正式后端域名（HTTPS）。
5. 通过 TestFlight 内测后再提交审核上架。
